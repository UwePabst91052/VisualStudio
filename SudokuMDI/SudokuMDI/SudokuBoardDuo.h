...
class CSudokuBoardDuo :
	public CSudokuBoard
{
    DECLARE_SERIAL(CSudokuBoardDuo)
public:
	CSudokuBoardDuo(void);
	CSudokuBoardDuo(int nBoardDim);
	virtual ~CSudokuBoardDuo(void);
...
protected:
	...
	CObArray mySudokuUnion;


private:
	// helper functions
	VectorResult_t BoardToVector(CPoint ptBoard , CPoint& ptVect1, CPoint& ptVect2, CPoint& ptVect3);
	CSudokuField* GetDisplayField(CPoint ptBoard);
	void AddSudokuField(CSudokuField* aField);
};
